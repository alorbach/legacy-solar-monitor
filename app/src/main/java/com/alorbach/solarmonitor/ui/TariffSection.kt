...
fun TariffSection(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        rows.forEachIndexed { index, row ->
            OutlinedTextField(
                value = row.from,
                onValueChange = { value -> rows = rows.toMutableList().also { it[index] = row.copy(from = value) } },
            ...
            OutlinedTextField(
                value = row.to,
                onValueChange = { value -> rows = rows.toMutableList().also { it[index] = row.copy(to = value) } },
                modifier = Modifier.fillMaxWidth(),
